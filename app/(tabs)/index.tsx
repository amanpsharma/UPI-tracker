...
export default function Dashboard() {
  ...
  // Build chart bars
  const chartDays = eachDayOfInterval({ start: subDays(new Date(), 6), end: new Date() });
  const trendMap: Record<string, number> = {};
  trend.forEach(t => { trendMap[t.date] = t.total; });
          ...
          <View style={styles.headerLeft}>
            ...
            {pctChange !== null && (
              <View style={styles.compareRow}>
                ...
                />
                <Text style={[styles.compareText, { color: decreased ? '#16a34a' : '#dc2626' }]}>
            ...
            {chartBars.map((bar, i) => {
              ...
              return (
                <View key={i} style={styles.barCol}>
                  ...
                  />
                  <Text style={[styles.barDayLabel, bar.isToday && styles.barDayLabelToday]}>
                    {bar.label}
                  </Text>
                </View>
              );
        ...
        <View style={styles.recentSection}>
          ...
          {recent.length === 0 ? (
            <View style={styles.emptyBox}>
              <MaterialCommunityIcons name="receipt-text-outline" size={32} color="#e5e7eb" />
...
const styles = StyleSheet.create({
  ...
  catCard: {
    backgroundColor: '#fff',
    borderRadius: 14, padding: 14, minWidth: 120,
    gap: 8,
    shadowColor: '#000',
    shadowOffset: { width: 0, height: 1 },
    shadowOpacity: 0.04,
