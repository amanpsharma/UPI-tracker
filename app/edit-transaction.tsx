...
export default function EditTransaction() {
  const params = useLocalSearchParams<{
    ...
    note: string; upiId: string; category: string; type: string;
  }>();
  ...
  const isValid = !!amount.trim() && parseFloat(amount) > 0 && !!recipient.trim();

  const today = new Date();
  const dateLabel = isToday(today) ? 'Today' : isYesterday(today) ? 'Yesterday' : format(today, 'MMM d');
  const timeLabel = format(today, 'HH:mm');

  const handleSave = async () => {
    const parsed = parseFloat(amount);
    if (!recipient.trim()) { setSnack('Recipient name is required.'); return; }
      ...
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        ...
        >
          ...
          <View style={styles.toggle}>
            ...
            >
              <Text style={[styles.toggleText, txType === 'sent' && styles.toggleTextActive]}>Expense</Text>
            </TouchableOpacity>
            <TouchableOpacity
              style={[styles.toggleOption, txType === 'received' && styles.toggleActive]}
              onPress={() => setTxType('received')}
            >
              <Text style={[styles.toggleText, txType === 'received' && styles.toggleTextActive]}>Income</Text>
            </TouchableOpacity>
          </View>
          ...
          <View style={styles.field}>
            ...
            >
              ...
              <MaterialCommunityIcons name={showCatPicker ? 'chevron-up' : 'chevron-down'} size={18} color="#9ca3af" />
            </TouchableOpacity>
            {showCatPicker && (
              <View style={styles.catDropdown}>
                {CATEGORIES.map((cat, i) => {
                  ...
                  return (
                    <TouchableOpacity
                      key={cat}
                      style={[styles.catOption, isSelected && styles.catOptionSelected, !isLast && styles.catOptionBorder]}
                    ...
                    >
                      ...
                      {isSelected && <MaterialCommunityIcons name="check" size={16} color="#16a34a" />}
                    </TouchableOpacity>
                  );
                })}
              </View>
...
const styles = StyleSheet.create({
  ...
  toggle: {
    flexDirection: 'row', backgroundColor: '#e8e7e4',
  ...
  amountPrefix: {
    fontSize: 26, color: '#9ca3af', fontWeight: '500',
    fontFamily: 'Inter_500Medium', marginRight: 4,
  },
  amountInput: {
    fontSize: 56, fontWeight: '800', color: '#111827',
    fontFamily: 'GeistMono_700Bold', padding: 0, minWidth: 60,
  },

  field: { gap: 6 },
  fieldLabel: {
    fontSize: 11, fontWeight: '700', color: '#9ca3af',
    letterSpacing: 0.7, fontFamily: 'Inter_700Bold',
  },
  ...
  selectText: { flex: 1, fontSize: 15, color: '#111827', fontFamily: 'Inter_400Regular' },
  ...
  catOptionText: { flex: 1, fontSize: 15, color: '#111827', fontFamily: 'Inter_400Regular' },
  ...
  bankChip: {
    ...
    borderRadius: 10, backgroundColor: '#e8e7e4',
  },
  bankChipActive: { backgroundColor: '#111827' },
  bankText: { fontSize: 14, fontWeight: '600', color: '#374151', fontFamily: 'Inter_600SemiBold' },
  bankTextActive: { color: '#fff' },

  bottomRow: {
    flexDirection: 'row', paddingHorizontal: 20, paddingTop: 12, paddingBottom: 16,
    gap: 12, backgroundColor: BG,
    borderTopWidth: 1, borderTopColor: '#e8e7e4',
  },
  cancelBtn: {
    flex: 1, paddingVertical: 15, borderRadius: 14, alignItems: 'center',
    backgroundColor: '#fff', borderWidth: 1, borderColor: '#e5e7eb',
  },
  cancelText: { fontSize: 15, fontWeight: '600', color: '#374151', fontFamily: 'Inter_600SemiBold' },
  primaryBtn: {
    flex: 2, paddingVertical: 15, borderRadius: 14, alignItems: 'center',
    backgroundColor: '#111827',
  },
  primaryBtnDisabled: { backgroundColor: '#d1d5db' },
  primaryText: { fontSize: 15, fontWeight: '600', color: '#fff', fontFamily: 'Inter_600SemiBold' },
});
